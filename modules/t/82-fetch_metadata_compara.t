...
ok(defined $md->compara() && ref $md->compara() eq 'ARRAY', "compara now expanded");
my @comparas = grep {$_->method eq 'PROTEIN_TREES' && $_->division() ne 'EnsemblPan'} @{$md->compara()};
...
is($is_found, 1,"metadata ".$md->species()." found in compara ".$comparas[0]->method()."/".$comparas[0]->division());

diag("Fetching all genome info with pan compara");
my @mds = grep{$_->division() ne 'Ensembl'} @{$gdba->fetch_with_pan_compara()};
$nAll = scalar @mds;
diag("Fetched $nAll GenomeInfo");
ok($nAll>0, "At least 1 GenomeInfo found");
...
ok(defined $md->compara() && ref $md->compara() eq 'ARRAY', "compara now expanded");
@comparas = grep {$_->method eq 'PROTEIN_TREES' && $_->division() eq 'EnsemblPan'} @{$md->compara()};
